class SubalphasController < ApplicationController
  ...
  def new
    @alpha = @project.alphas.new
    method_definition = @project.method_definition
    @alpha_definitions = method_definition.alpha_definitions
    @alpha_definitions = @alpha_definitions.reject do |definition|
      definition.parent_id.nil?
    end
  end

  def create
    @alpha = @project.alphas.new(alpha_create_params)
    if @alpha.save
      flash[:notice] = "Creation successful"
      redirect_to subalpha_path(@alpha, project_id: @project.id)
    end
  end
  ...
  def update
    ...
    redirect_to subalpha_path(@alpha, project_id: @alpha.project_id)
  end
